;Scan driver auto hotkeys for Ricoh MP1350 Twain v.4
...
{
    global scan_driver
    fn1Click(scan_driver, 476, 526)
}

pubFnPreview() ;OK
{
    global scan_driver
    fn1Click(scan_driver, 354, 526) 
}

pubFnQuitScanDriver() ; FIXME
{    
    global scan_driver
...
ScanDriver_wb()  ;brightness  363,352
{
}

ScanDriver_wc()  ;contrast   363, 378
{
}
...
ScanDriver_we()  ;;export a driver library
{
    ...
    FileSelectFile, selected, S24, %RicohMP1350%, %SelectFileMsg%, Registry patches (*.reg)
    If ErrorLevel = 0
    {
        If RegExMatch(selected, "i)\.reg$")
            patch := selected
        else patch := selected . ".reg"
        Run, regedit /e %patch% "HKEY_CURRENT_USER\Software\Ricoh\Twain_V4\C:`%WINDOWS`%Twain_32`%Ricoh_V4`%TP1350\English"
    }
    fnSuspend()
    suspend
}
...
ScanDriver_wi()  ;;import a driver library 
{
    ...
    FileSelectFile, selected, 3, %RicohMP1350%, %SelectFileMsg%, Registry patches (*.reg)
    If ErrorLevel = 0
    {
...
pubFnGetScanDriver()
{
    ...
    WinGetTitle, scan_driver, KODAK i730 Scanner,
